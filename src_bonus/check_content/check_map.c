...
/*   Updated: 2022/05/05 10:19:23 by xxxxxxx          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_map_content(t_cub *cub, int *spawn)
{
	...
	while (++i < cub->nb_line)
	{
		...
		while (++j < cub->nb_cols)
		{
			...
			if (cub->map[i][j] == -666)
				return ;
			if (check_map_plus(cub, i, j, spawn) == -1)
				return ;
...
static void	check_map_border(t_cub *cub, int i, int j, int *spawn)
{
	if ((cub->map[i][j] == FLOOR || cub->map[i][j] == SOPHIE || cub->map[i][j]
		== SPAWN || cub->map[i][j] == EW_CLOSE || cub->map[i][j] == NS_CLOSE)
		&& !(j + 1 <= cub->nb_cols && cub->map[i][j + 1]
